...
Experimental PoC (lives under `support/`, **not wired into `StatChart`** yet). The
importable module is `src/signum/engine/surface3d.py` with echarts vendored in
`src/signum/vendor/`. A rotatable WebGL surface for vol surfaces, term-structure /
continuous-time quant models, themed from signum's `THEMES` to match Chart / StatChart.

> echarts pinned to **5.4.3** — echarts-gl 2.0.9 renders blank with echarts 5.5.x.
...
```
python support/surface_demo.py
```
